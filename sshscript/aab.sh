...
log_path=/home/xiaoayong/work/jinli_qingdianshang/20190812
domain="mail.qq.com"
email="843150233@qq"
...
maketime=`date +%Y-%m-%d" "%H":"%M`
logdate=`date -d "yesterday" +%Y-%m-%d`
for file in /home/xiaoayong/work/jinli_qingdianshang/`date -d "yesterday" +%Y-%m-%d`/* #日志文件路径
do
    if test -f $file #如果是文件，统计异常数量，并输出到ex.log
    then
         MplhPv=`cat * | grep '"nm":"GIONEE_TAOBAO_HELPER_DIALOG_SHOW"' | wc -l  `

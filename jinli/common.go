...
func init() {
	...
	logReadUrl = myConfig.Read("log", "jinLiLogReadPath")
	logWriteUrl = myConfig.Read("log", "jinLiLogWritePath")
	...
	for _, file := range files {
		...
		for {
			...
			if len(line) != 0 {
				...
				mapString := strconv.Itoa(lineNum) + "_" + strconv.Itoa(fileNum)
				logDataOne[mapString] = JsonFormatOne{m1, nm, eplatform}
